...
# スクリプトディレクトリを取得（絶対パス化）
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# プロジェクトディレクトリに移動して実行
cd "$SCRIPT_DIR/../../focus-flow-capacitor"
...
# 1.5. CSSクラス定義チェック (エラー時中断)
echo -e "\n${YELLOW}1.5/5: CSSクラス定義をチェック中...${NC}"
bash "$SCRIPT_DIR/check-css-classes.sh"
if [ $? -ne 0 ]; then
    echo -e "\n${RED}未定義CSSクラスが検出されました。処理を中断します。${NC}"
    exit 1
fi
echo -e "${GREEN}CSSクラス定義チェッククリア！${NC}"

# 1.6. useEffectの使用状況チェック (警告のみ)
echo -e "\n${YELLOW}1.6/5: useEffectの使用状況をチェック中...${NC}"
...
# 2. 単体・結合テスト (Vitest)
echo -e "\n${YELLOW}2/5: 単体・結合テスト (Vitest) を実行中...${NC}"
npm run test:run
if [ $? -ne 0 ]; then
...
# 3. E2Eテスト (Playwright)
echo -e "\n${YELLOW}3/5: E2Eテスト (Playwright) を実行中...${NC}"
npm run test:e2e
if [ $? -ne 0 ]; then
...
echo -e "${GREEN}チェック項目: ESLint + CSSクラス定義 + テスト + E2E + ビルド${NC}"
