...
#### 内容
絵文字は端末依存があるためUIが保証できない。マテリアルデザインアイコンにしたい。（2025-07-31更新：目の絵文字が気持ち悪いので早めに更新したい。）
#### 緊急度/重要度
低/大
#### progress-management転記
未
...
低/大
#### progress-management転記
済（T018として中優先度登録済み、Phase 2.3で実装予定）
...
低/大
#### progress-management転記
済（T019として中優先度登録済み、Phase 2.3で実装予定）
...
低/中
#### progress-management転記
済（T020として低優先度登録済み、フォーカスモード統合要検討）
...
中/大
#### progress-management転記
済（T016として高優先度登録済み、Phase 2.2b並行対応予定）
...
中/大
#### progress-management転記
済（T017として高優先度登録済み、Phase 2.2b並行対応予定）
...
低/中
#### progress-management転記
済（T009として緊急昇格・統合設計済み、Builder実装待ち）
...
低/中
#### progress-management転記
済（T021として低優先度登録済み、ゲーミフィケーション統合時に検討）
...
低/大
#### progress-management転記
済（T022として低優先度登録済み、Phase 2.4以降で検討・feature-roadmapで管理）
...
低/大
#### progress-management転記
済（T023として低優先度登録済み、Phase 2.4以降で検討・feature-roadmapで管理）

### モバイルレイアウト　タスクメモがタスク別に保存できてない
#### 内容
モバイルレイアウトで発火するタスクメモ保存機能が、どうもタスク横断で参照されている様子。デスクトップレイアウトの自動保存時に発火するものと同じ保存機能を使ってほしい。
#### 緊急度/重要度
大/大
#### progress-management転記
済（T010として緊急登録済み、T009と統合対応・Builder実装指示拡張）

### タスク登録フォーム ラベル1日ズレ
#### 内容
朝7時にアプリを開くと、今日の日付で登録しようとすると日付変更ボタンが「明日」と表示される。明日の日付で登録しようとすると「2日後」と表示される。1日ズレている（テスト登録は実際の日付で登録される）
今日・明日以外は実際の日付を表示してもらえたほうが分かりやすいと思う。
#### 緊急度/重要度
低/中
#### progress-management転記
済（T024として低優先度登録済み、日付表示ロジック修正）

### モバイル・タブレットレイアウト　タスクメモ/デイリーメモ入力時、キーボードが表示されていればそれに合わせてメモ欄を縮小する　
#### 内容
現在タスクメモ・デイリーメモを入力時、ある程度長くなってくるとキーボードに隠れてしまい見えなくなってしまう。入力欄を狭めることでこれを回避してほしい。
#### 緊急度/重要度
低/中
#### progress-management転記
済（T025として低優先度登録済み、キーボード表示時の入力欄調整）

### playwright 日本語が正しく表示されない
#### 内容
linux chromiumに日本語フォントが読み込まれていない様子。オーソドックスな日本語フォントを導入したい。作業品質・UXの確認のため開発手順にマニュアル作成まで組み込みたいので優先度が高い
#### 緊急度/重要度
中/大
#### progress-management転記
未
...
### タスク毎の作業実績を記録したい
#### 内容
作業実績の記録機能が欲しい。フォーカスモードで作業した時間（実績）ベースで判断したい。また、残りの見積時間も「未完了タスクの残時間の合計」としたい。（各タスクの残時間は「見積時間-実績時間」か「0分」の大きい方としたい。）
#### 緊急度/重要度
中/中
#### progress-management転記
未