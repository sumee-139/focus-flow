...
## 作業の進め方（TDD厳守＋タスクステータス管理）　**絶対遵守**
1. 引き継ぎ確認 → `planner/handover.md`を読む、タスクステータス確認（🔴🟢✅⚠️）
2. 作業ブランチ確認 → 自身の作業ブランチを確認し、適切なブランチを作成・移動。作業ブランチは`builder/note.md`に記載。
2. **🔴 Red Phase** → 失敗するテストを先に書く（実装より前）、タスクは🔴のまま
3. **🟢 Green Phase** → テストを通す最小限の実装（仮実装OK）、テスト通過後🔴→🟢に更新
4. **🔵 Refactor Phase** → リファクタリング（テストが通る状態を維持）、完了後セマンティックコミット(`.claude\commands\semantic-commit.md`参照)し、🟢→✅に更新
...
7. ユーザー用手順書作成 → `manual`ディレクトリに、操作手順を示すmarkdownファイルを作成する。この時playwright MCPを利用して、操作する画面をスクリーンショットで示す。
...
**各ステップはすべてToDoとして管理し、ステップが完了する度にステータスを更新すること**
